...
function RenderTopic() {
	...
	uParse('.topic-content', {
		...
		'liiconpath': WebsitePath + '/static/editor/themes/umeditor-list/'//使用 '/' 开头的绝对路径
	});
...
function InitEditor() {
	...
	window.umeditor_CONFIG['elementPathEnabled'] = false;
	window.umeditor_CONFIG['toolbars'] = [
		[
			'fullscreen',
			'source',
			'|',
...
function ReplyCtrlAndEnter(Event) {
	...
	if (Event.ctrlKey && Event.keyCode === 13) {
		$("#ReplyButton").click();
		Event.preventDefault ? Event.preventDefault() : Event.returnValUM = false;//阻止回车的默认操作
	}
}
...
function EditPost(PostID) {
	...
	$("#p" + PostID).hide();
	window.umeditor_CONFIG['textarea'] = 'PostContent' + PostID;
	UM.getEditor('edit' + PostID, {
		onready: function () {
			UM.getEditor('edit' + PostID).setContent(PostContentLists['p' + PostID]); //将帖子内容放到编辑器里
		}
	});
	$("#edit" + PostID).show();
	if ($("#edit_button" + PostID) && $("#edit_button" + PostID).length == 0) {
		$("#edit" + PostID).append('<div id="edit_button' + PostID + '"><p></p><p><input type="button" valUM=" ' + Lang['Edit'] + ' " class="textbtn" id="EditButton' + PostID + '" onclick="JavaScript:SubmitEdit(' + PostID + ');">&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" valUM=" ' + Lang['Cancel'] + ' " class="textbtn" onclick="JavaScript:DestoryEditor(' + PostID + ');"></p>');
	}
...
function DestoryEditor(PostID) {
	UM.getEditor('edit' + PostID).destroy();
	$("#p" + PostID).show();
...
function SubmitEdit(PostID) {
	...
	$.ajax({
		...
		data: {
			...
			Type: 2,
			Action: 'Edit',
			Content: UM.getEditor('edit' + PostID).getContent()
		},
		cache: false,
		dataType: "json",
		type: "POST",
...
function EditPostCallback(PostID) {
	this.Success = function (Json) {
		if (Json.Status === 1) {
			document.getElementById('p' + PostID).innerHTML = UM.getEditor('edit' + PostID).getContent();
			PostContentLists['p' + PostID] = UM.getEditor('edit' + PostID).getContent();
...
function ReplyToTopic() {
	if (!UM.getEditor('editor').getContent().length) {
		...
		$.ajax({
			...
			data: {
				FormHash: document.reply.FormHash.valUM,
				TopicID: document.reply.TopicID.valUM,
			...
			success: function (data) {
				if (data.Status === 1) {
					...
					//清空编辑器
					//UM.getEditor('editor').execCommand('cleardoc');
					console.log(SavePostDraftTimer);
					$.pjax({
...
function Reply(UserName, PostFloor, PostID) {
	UM.getEditor('editor').setContent('<p>' + Lang['Reply_To'] + '<a href="' + location.pathname + '#Post' + PostID + '">#' + PostFloor + '</a> @' + UserName + ' :<br /></p><p></p>', false);
	UM.getEditor('editor').focus(trUM);
}

//引用某人
function Quote(UserName, PostFloor, PostID) {
	UM.getEditor('editor').setContent('<p></p><blockquote><a href="' + location.pathname + '#Post' + PostID + '">#' + PostFloor + '</a> @' + UserName + ' :<br />' + PostContentLists['p' + PostID] + '<blockquote>');
	UM.getEditor('editor').focus(false);
}

//Save Draft
function SavePostDraft() {
	try {
		if (UM.getEditor('editor').getContent().length >= 10) {
			localStorage.setItem(Prefix + "PostContent" + TopicID, UM.getEditor('editor').getContent());
		}
	} catch (oException) {
...
function StopAutoSave() {
	...
	UM.getEditor('editor').execCommand("clearlocaldata"); //清空umeditor草稿箱
}

function RecoverContents() {
	var DraftContent = localStorage.getItem(Prefix + "PostContent" + TopicID);
	if (DraftContent) {
		UM.getEditor('editor').setContent(DraftContent);
	} else {
		UM.getEditor('editor').execCommand('cleardoc');
	}
}