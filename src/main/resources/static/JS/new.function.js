...
function TagsListAppend(TagName, id) {
	$("#TagsList").append('<a href="###" onclick="javascript:addTagAfterMaxValidation(\'' + TagName + '\',' + id + ');" id="TagsList' + id + '">' + TagName + '+</a>');
	//document.NewForm.AlternativeTag.focus();
}

function AddTag(TagName, id) {
	if (CheckTag(TagName, 1)) {
		$("#SelectTags").append('<a href="###" onclick="javascript:TagRemove(\'' + TagName + '\',' + id + ');" id="Tag' + id + '">' + TagName + '×</a>');
		$("#TagsList" + id).remove();
	}
	//document.NewForm.AlternativeTag.focus();
	$("#AlternativeTag").val("");
	if ($("input[name='Tag[]']").length === 5) {
		$("#AlternativeTag").attr("disabled", true);
		//$("#AlternativeTag").attr("placeholder", Lang['Tags_Too_Much'].replace("{{5}}", 5));
	}
}
